...
///
/// Designed against a real precedent. In April 2026 Drift lost $285M after months of
/// social engineering yielded admin control and the attacker whitelisted fake collateral.
/// This contract is structurally the same object: an allowlist whose compromise drains
/// every vault at once. So it carries two defences.
///
///   1. A timelock. Additions are queued publicly and cannot execute for `delay`. A
...
///      never add. Least authority in the direction that matters: the emergency key can
///      only ever shrink the attack surface.
///
...
contract VenueWhitelist {
    ...
    mapping(address => Queued) public queued;
    ...
    /// Governance is the normal path for removal; the guardian is the 3am path.
    ...
    /// Deliberately immediate: a suspected-compromised guardian needs replacing faster
    /// than a timelock allows.
