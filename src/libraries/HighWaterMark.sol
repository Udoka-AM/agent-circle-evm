...
///
/// The correctness-critical part of the system. Fees accrue only on profit above a
/// vault's all-time-high balance. Without that, a volatile agent earns on every up-swing
/// and returns nothing on down-swings, letting a builder farm variance and extract fees
/// from a trader who ended up flat or down.
///
/// Deliberately a pure library over a plain struct: no storage, no external calls, no
/// access control. Every rule here is unit-testable in isolation, which is the point.
/// This is the logic most worth proving and least worth trusting to review.
library HighWaterMark {
    ...
    function assess(State memory s, uint16 performanceFeeBps, uint16 builderSplitBps)
        internal
        pure
        returns (State memory, FeeSplit memory split)
    {
        ...
        split.total = (profit * performanceFeeBps) / Constants.BPS;
        split.builderCut = (split.total * builderSplitBps) / Constants.BPS;
        // Derived by subtraction rather than a second multiplication, so the two cuts
        // always sum to exactly `total` whatever the rounding.
        split.platformCut = split.total - split.builderCut;

        s.balance -= split.total;
        // Post-fee, so the same profit can never be charged twice.
    ...
    function onWithdraw(State memory s, uint256 amount) internal pure returns (State memory) {
        ...
        // so a full withdrawal would underflow. Clamping to zero is correct: there is no
        // capital left for a loss-carryforward to apply to.
    ...
    function drawdownBps(State memory s) internal pure returns (uint256) {
        ...
        return ((s.highWaterMark - s.balance) * Constants.BPS) / s.highWaterMark;
    }
}
